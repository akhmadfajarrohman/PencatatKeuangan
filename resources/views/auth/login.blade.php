...
<head>
    ...
    <title>Login — Dompet Harian</title>
...
<body class="h-full flex items-center justify-center p-4 antialiased selection:bg-emerald-500 selection:text-white" style="font-family: 'Inter', sans-serif;">
    ...
    <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-slate-400 text-xs mt-1">Masuk untuk mengelola catatan pengeluaran harian Anda.</p>
        </div>

        @if (session('success'))
            <div class="mb-5 p-3 bg-emerald-950/40 border border-emerald-800/40 text-emerald-400 text-xs rounded-xl flex items-center gap-2">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div class="flex items-center justify-between py-1">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="remember" class="w-4 h-4 bg-slate-950 border border-slate-800 rounded text-emerald-500 focus:ring-emerald-500 focus:ring-offset-0 focus:ring-0">
                    <span class="text-xs text-slate-400">Ingat saya</span>
                </label>
            </div>

            <!-- Submit -->
            <button type="submit" class="w-full bg-emerald-500 hover:bg-emerald-600 active:scale-[0.98] text-white font-semibold text-sm rounded-xl py-3 shadow-lg shadow-emerald-500/20 transition duration-150">
                Masuk
            </button>
        </form>
        ...
        <div class="mt-8 text-center border-t border-slate-800/80 pt-6">
            <p class="text-slate-500 text-xs">Belum punya akun? <a href="{{ route('register') }}" class="text-emerald-400 hover:text-emerald-300 font-semibold transition">Daftar Sekarang</a></p>
        </div>
    </div>
